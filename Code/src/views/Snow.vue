...
export default {
    ...
    methods: {
        createSnow () {
            this.$nextTick(() => {
                setInterval(() => {
                    ...
                    snow.setAttribute('x', String(this.getRandom(0.5, 0.6, 2)))
                    ...
                    snow.style.top = '-20px'
                    snow.style.opacity = String(this.getRandom(0.4, 0.9))
                    snow.style.color = this.isColor ? color : '#FFF'
                    snow.style.left = this.getRandom(0, window.innerWidth) + 'px'
                    snow.style.transform = `scale(${this.getRandom(0.4, 1)})`
                    // snow.style.filter = `drop-shadow(0 0 3px ${isColor.value ? color :'#FFF'})`
                    snow.style.textShadow = `0 0 10px ${this.isColor ? color :'#FFF'}`
                    snow.innerHTML = this.iconList[this.getRandom(0, 20, 1)]
        ...
        init () {
            this.$nextTick(() => {
                let snowDom = document.querySelectorAll('.snow')
                // if (!snowDom.length) clearInterval(timer)
                snowDom.forEach(v => {
                    if (+v.style.top.slice(0, -2) > window.innerHeight) v.remove()
                    if (+v.style.left.slice(0, -2) < -50 || +v.style.left.slice(0, -2) > window.innerWidth) v.remove()
        ...
        getColor () {
            let str = '#'
            for (let i = 0; i < 6; i++) {
                str += this.colorList[this.getRandom(0, 15, 1)]
            }
            return str
        }
    },
